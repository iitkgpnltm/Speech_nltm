% function [PitchCont,FirstPeakCont,HilbEnr]=HilbertPitch(henv,sampfreq,framesize,frameshift)




function [PitchCont]=HilbertPitch(henv,sampfreq,framesize,frameshift)
...
%
%   Output:
%           PitchCont: Pitch period contour
...
%       considered, else the frame is discarded.
...
for i=1:frameshift:length(henv)-framesize
   ...
   if(mvalue > 0.4)
        ...
        if(pitch>3)
            ...
            PitchCont(nfrms)=pitch;

            %FirstPeakCont(nfrms)=mvalue;
            %HilbEnr(nfrms)=enr;
...
%HilbeEnr=HilbEnr./(0.1*mean(HilbEnr));
nfrms;