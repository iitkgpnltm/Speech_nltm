...
if (in_array($fileActualExt, $allowed)) {
    if ($fileError == 0) {
        if ($fileSize > 0 && $fileSize < 4000000) {
			...
			$cmd = 'matlab -nosoftwareopengl -nosplash -nodesktop -sd ' . $prosody_dir . ' -r "prosodytrans('.'\''.$wav_path.'\''.');exit" -logfile prosodylog.txt';
...
if ($flag) {
    ...
    $fileOpen = fopen("./pitch_trans/output_file/Pitch_label.txt", "r");
    $fileRead = fread($fileOpen, filesize("./pitch_trans/output_file/Pitch_label.txt"));
    $result = array(
        "img_o" =>
